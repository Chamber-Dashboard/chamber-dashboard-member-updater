...
// when business name changes, check to see whether the business is already in the database
    ...
    $('#bus_name').keyup(function (evt) {
        ...
        $.post(url, data, function (response) {
          //jQuery("#business-picker").css("display", "block");
          jQuery("#business-picker").html(response);
        });

        if( $('#bus_name').val().length === 0 ) {
          jQuery("#business-picker").css("display", "none");
        }else{
          jQuery("#business-picker").css("display", "block");
        }
